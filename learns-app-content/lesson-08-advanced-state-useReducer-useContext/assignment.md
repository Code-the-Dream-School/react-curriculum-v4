...
> [!NOTE]
> We don't functionally update here. Functional updates are most useful when the new state relies on the previous state. Here, we want the new state (the data from our fetch request) to replace our previous state (the empty array we started with).
